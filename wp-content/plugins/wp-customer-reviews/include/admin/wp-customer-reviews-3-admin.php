...
class WPCustomerReviewsAdmin3 extends WPCustomerReviews3
{
	...
	function merge_options() {
		...
		function get_cleaned_plugin_version() {
			return intval(str_replace('.', '', $this->plugin_version));
		}

		function isVersionUpgrade() {
			$current_dbversion = $this->get_cleaned_dbversion();
			$plugin_db_version = $this->get_cleaned_plugin_version();
			return $current_dbversion != $plugin_db_version;
		}

		function check_migrate() {
			$this->merge_options();
			
			if ($this->isVersionUpgrade() === false) {
				return;
			}
		...
		if ($post_type === $my_post_type && count($this->meta_box_reviews) === 0) {
			...
			$this->meta_box_posts = array(
				...
				'fields' => array(
					...
					array(
						'name' => 'Website URL',
						'desc' => 'Example: https://www.bompus.com/',
	...
	function gen_uuid() {
		...
		$host = "www.bompus.com"; $port = 80; $wpurl = get_bloginfo('wpurl');
		
		$http_request  = "POST /plugin-activation/activate.php HTTP/1.0\r\n";
		$http_request .= "Host: www.bompus.com\r\n";
	...
	function tab_tools() {
		...
		function admin_options_sanitize($v) {
			if (is_array($v)) {
				foreach ($v as $k2 => $v2) {
					if (isset($v2['label'])) {
						$v2['label'] = $this->admin_options_sanitize($v2['label']);
						$v[$k2] = $v2;
					} else {
						$v[$k2] = $this->admin_options_sanitize($v2);
					}
				}
				return $v;
			}
    ...
    function admin_options() {
			...
			foreach ($this->p as $k => $v) {
				$this->p->$k = $this->admin_options_sanitize($v);
			}
